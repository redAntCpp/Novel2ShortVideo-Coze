# interface_level（1）——小说整体分析主流程搭建

我们先假定用户会上传一个小说，我们首先第一步，是需要对小说进行初步的分析，否则用户可能上传一个微小说或者一个不完整的小说，甚至不适合改编的小说，此时就不必浪费后续的token了。

工作流名称：interface_level

工作流描述：用户会上传一部小说，这里需要完成以下工作：

1. 理解小说主要内容
2. 判断小说规模
3. 是否值得改编（我们可以初步设定一个打分规则）
4. 为下一层的工作做准备。输出结构化的内容。

## 整体流程初步浏览

![image-20260510193433480](../images/doc/image-20260510193433480.png)
...
![image-20260510193549148](../images/doc/image-20260510193549148.png)

在开始节点处，将变量类型设置为Doc。表示我们我们可以将Doc文件上传，并作为流程的开始数据源。这里**强烈建议**使用doc或者docx格式的文件上传，虽然官方说支持txt格式，但是实测txt格式上传后，读取的文本模式编码进行了改变，因此读取出来的内容是乱码，虽然不影响大模型进行解读，但是后续如果要对原始文本做处理，则会破坏文本的格式，造成不可逆的后果。示例如下：

txt版本：

![image-20260510194425548](../images/doc/image-20260510194425548.png)

doc版本：

![image-20260510194455463](../images/doc/image-20260510194455463.png)

这里简单说明一下，文本读取节点的内容“

![image-20260510194610254](../images/doc/image-20260510194610254.png)

之所以不能直接把开始节点直接连接LLM节点，是因为我们把文件上传后，在后台记录的是一个文件的地址（即URL）。类似我们把文件上传给网盘，用户访问的实际上是这个URL。因此我们要处理里面的内容，需要首先把URL的内容读取出来，因此需要一个读取URL内容的节点进行辅助。根据插件的说明，读取的内容会出现在data字段返回。

因此我们将LLM中的输出，命名为story_text，并将它的数据来源设置为这个节点的输出字段data：

![image-20260510195128155](../images/doc/image-20260510195128155.png)

**注意：后续的所有节点的输出跟输入的对接，都必须保证数据类型的一致性**

## 第二步：小说分析（LLM节点）

有了小说的全文输入，我们就可以对小说进行分析了。这个LLM节点的主要目的，是判断小说是否值得改编，进行评分，给出合适的分集策略。并对小说进行分类。
...
11. 【输出格式要求】
请严格按照JSON结构（注意，所有的key用英文表达）输出1-9的分析结果，不要添加多余解释.示例如下：
{
...
随后，我们将LLM节点的输出设置为analyze_result，并将输出连接到输出节点output：

![image-20260510200616562](../images/doc/image-20260510200616562.png)

至此，该流程核心模块搭建完成。

大家可能对提示词中出现的```{{time_background}}```、```{{theme_plot}}```等内容有疑问。这个下篇详细解释。
